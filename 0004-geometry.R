...
irunning <- function (to, length.run=2)
        vapply(
            seq.int(to - length.run + 1L),
            function (from)
                    seq.int(
                        from=from,
                        length.out=length.run),
            n %|% integer)
...
i.closed <- function (i)
    ...
    Doc$i.closed <- '
        i.closed returns a modified copy of the index-vector
        argument. The return is identical to the argument,
        except that the first element is repeated at the end of
        the return.  Originally intended to aid in the analysis
        of closed polygons specified by a vector of indices of
        points. **** NOT INTENDED **** to be called with a
        null-vector argument.'
...
quadrances.starburst <- function (m)
    ...
    Doc$quadrances.starburst <- '
        quadrances.starburst returns the quadrances of the
        starburst represented by the matrix-of-points argument.
