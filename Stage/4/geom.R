...
max_pp <- rapply. %|% argswap %<=% max #TAGS upper right corner bounding box
min_pp <- rapply. %|% argswap %<=% min #TAGS lower left corner bounding box origin

max_pt <- max_pp #DEPRECATED USE max_pp
min_pt <- min_pp #DEPRECATED USE min_pp

bb.pp <- function (points) #TAGS bounding box
        applyf(points, min_pp %,% max_pp) %|% rbind_l

bb.pt <- bb.pp #DEPRECATED USE bb.pp
...
ipp.bb <- function (bb, .ibb=capply1to1(bb, ceiling %,% floor)) #TAGS points bounding box
        .ibb %|% diff_inclusive %|% m.indices %|% pred %|% t + .ibb[1,]

ipt.bb <- ipp.bb #DEPRECATED USE ipp.bb

    Doc$ipp.bb <- '
        ipp.bb returns the integer-valued points within
