...
'""
pred #MOVED TO neatoArithmetic0 #TAGS predecessor subtract 1L integer -1 1-
succ #MOVED TO neatoArithmetic0 #TAGS successor        add 1L integer +1 1+
rmna #MOVED TO THE rmALLTHE PACKAGE
rmz #MOVED TO THE rmALLTHE PACKAGE
""'

concatenate <- function(h, ..., collapse="") #TAGS string character
        paste0(c(h, list(...) %|% unlist), collapse=collapse)

`%//%` <- concatenate #TAGS string character

`%/ /%` <- HSPACE %=:% "collapse" %v% concatenate #TAGS string character

`% //%` <- `%/ /%` #DEPRECATED Use `%/ /%` #TAGS string character

    Doc$`%//%` <- '
        The `%/*/%` (e.g., `%//%`, `%/ /%`) family of functions
        concatenate character vector(s) into character strings.
        ...
        + c("I", "[do not]", "think", "they", "exist.")  

        "Rodents of unusual size? I [do not] think they exist."
...
uniques <- unique %O% (F %=:% "na.last" %v% sort)
    ...
    Doc$uniques <- '
        uniques returns the unique items of the atomic vector
        argument, sorted. Any NA values are represented by the
        first element of the return.
        
        TIP: apply rest to the return to discard NA-values.'
        
nunique <- unique %O% `#`

    Doc$nunique <- '
        nuniques returns the number of unique values---including
        NA-values, if present---in the vector argument.' 
...
NULLs <- nulls #DEPRECATED USE nulls
...
interlace <- rbind %O% as.vector
    ...
    Doc$interlace <- '
        interlace interlacs the parallel atomic vector arguments
        into a single vector.

        interlace(1:6, LETTERS[1:6])
...
.lo <- -1:0 # MNEMONIC -10
ol   <-  0:1 # MNEMONIC:   01
Ol <- ol # ZERO ONE (SEE: "THE SECOND RENAISSANCE PART I" IN THE ANIMATRIX (FILM))
lo <- 1:0 #MNEMONIC 10
lO <- lo
.lol <- -1:1 # MNEMONIC: -101
.lOl <- .lol
.ll  <- c(-1L, 1L) # MNEMONIC: -11 # HISTORY 2019-09-18 WAS FORMERLY double
l.l <- .ll %|% rev # HISTORY 2019-09-18 WAS FORMERLY doublev
...
na <- function (length=1L, mode="logical")
    ...
    Doc$na <- '
        na is similar to the vector function, except that 1) the
        order of arguments is swapped and 2) the elements of the
        return are NA of the mode specified.

        The rationale for the different argument order is that
        when called with a single, unnamed argument, the result
        is a vector of proper NA values (as opposed to, e.g.,
        NA_integer_), and it is not necessary to apply two,
        unnamed arguments nor a single, named argument to
        achieve this.'
...
### anonymize
'""
anonymize.v #DELETED USE anon:anon
anonymize.a #DELETED USE anon:anon
anonymize.m #DELETED USE anon:anon
anonymize.df #DELETED USE anon:anon
.anonymize #DELETED USE anon:anon
anonymize #DELETED USE anon:anon
""'
...
beyond2 <- function (interval, x) x < interval[1] | interval[2] < x
    ...
    Doc$between <- '
        ...
        of v (arg 3), tells whether that element is between the
        single value x (arg 1) and the single value y (arg 2).
        The interval is considered to be **** INCLUSIVE AT BOTH
        ENDS. ****'
...
range.check1 <- function (LENGTH, i)
    ...
    Doc$range.check1 <- '
        range.check1 returns a logical vector indicating for
        each element of the integer-valued index vector (arg 2)
        is in the range of a vector of the specified LENGTH (arg
        1).'
...
nice.i <- function (LENGTH, i) { #TAGS range check valid index
    ...
    Doc$nice.i <- '
        nice.i returns a modified copy of the integer-valued
        index (arg 2) of a vector of the LENGTH (arg 1) given.
        The return is the same as the given index, except that
        any invalid indices are replaced with NA.
        
        > nice.i(3, -3:6)

        [1] NA NA NA NA  1  2  3 NA NA NA'

among.indices.of <- function (X, i) #TAGS range check
        range.check1(X %|% `#`, i)
...
block.extract <- function(x, from, to)
        x[nice.i(x %|% `#`, from:to)]

    Doc$block.extract <- '
        block.extract returns an atomic vector whose elements
        are taken from the atomic vector argument (arg 1).
        Elements are taken according the the integer index
        from:to. Indices falling outside the dimension of arg 1
        map to NA-values in the return.

        > block.extract(c("AS", "YOU", "WISH"), 0, 4)
        ...
        removed optional na parameter. NA-values will be NA of
        the type of the vector argument.
        '
...
seq_symmetric <- function(to=0L, center=0L)
		center + seq(from=-to, to=to)

seq0 <- seq %<=% 0

'""
seqZp #REMOVED USE seqN
""'
...
i2.index.n <- function(m, n) {
    ...
    Doc$index.factors <- '
        In the family of functions index.factors.dim, n.index.i,
        and i.index.n, factors refers to an integer vector where
        each element contains the number of elements contained
        by the corresponding "level" of a hypothetical array.
        For example, a matrix of 3 columns and 2 rows would have
        factors ic(1, 2) and a 3 x 3 x 3 "cube" array would have
        factors of ic(1, 3, 9).'

    Doc$index.factors.dim <- '
        Given the dimensions of an array, returns the
        corresponding factors.
        
        see also doc(index.factors)'
...
' #ALTERNATE #TODO BENCHMARK
which1rev <- function(b) {
    i <- b %|% `#`
    while (0 < i) {
        if (b[i]) return(i)
        i[1] <- i[1] - 1L }
...
'""
`%mod1%` #MOVED TO neatoArithmetic0
""'
...
degenerate <- typeof %O% vector
    ...
    Doc$degenerate <- '
        degenerate returns an empty vector of the same type as
        the argument.'

count <- as.logical %O% which %O% length

count.v <- count #DEPRECATED USE count

    Doc$count <- '
        count returns the number of nonzero values. C.f. Fortran
        COUNT.'
