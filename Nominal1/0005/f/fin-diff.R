...
biv <- placeholder #TAGS binary divergence (2-D)

biv2 <- function(m) #TAGS binary divergence (2-D)
    anon(m %|% chalf.edges.m | m %|% rhalf.edges.m)

positive.edge2 <- function(mb)
        mb %|% biv2 & ! mb

    Doc$positive.edge2 <- '
        positive.edge2 returns a logical matrix indicating which
        elements are both FALSE and have at least one neighbor
        that is TRUE.
    
        negative.edge2 returns a logical matrix indicating which
        elements are both TRUE and have at least one neighbor
        that is FALSE.
        
        Reference https://en.wikipedia.org/wiki/Signal_edge
        '

negative.edge2 <- function(mb)
        mb %|% biv2 & mb

xorr2.m <- biv2

wall2 <- function(m) m %|% biv2 & m

    Doc$wall2 <- '
        wall2 it intended for conceptually "filled" logical
        raster regions indicated by TRUE values.  returns a
        same-sized raster with the inner boundary rasters (the
        "cell wall") set to TRUE.'

brad <- placeholder #TAGS binary grad

burl <- placeholder #TAGS binary curl
