...
introduced <- function(X) { #TAGS names alignment populate extend
    ...
    Doc$introduced <- '
        Returns a copy of the compound list argument.  The
        return contains slots for every name that is present
        among the inner lists of the argument.

        Provided that all inner lists of the argument are named,
        the inner lists of the return may be interpreted as
        being aligned by name.

        **** INTENDED FOR **** a compound list where all
        elements of the inner list are named.

        NOTES

        to introduce: "Make (someone) known by name to another
        in person, especially formally" [OED].

        REFERENCES

        [OED] (downloaded 2019-12-06) https://www.lexico.com/en/
...
list_frame <- function(nrow, ncol=NULL, .names=NULL) #TAGS data.frame
    ...
    Doc$list_frame <- '
        list_frame returns a compound list.  The nrow argument
        specifies the number of elements of each inner list.  If
        the .names argument is specified, these are the names of
        the elements of the return.  Alternatively, if ncol is
        specified, this is the number of elements of the return.

        Originally developed to facilitate working with
        heterogeneous lists.  See also transpose.by.name.
    '
...
transpose.by.name <- function(X) {
    ...
    Doc$transpose.by.name <- '
        transpose.by.name transposes a compound list where each inner list is named.
        The return contains one element per unique name found among the inner lists of the argument.
        The inner lists of the return contain one element per outer list of the argument.
        
        The call

        transpose.by.name(list(list(A=1, B=2), list(C=3)))

        returns the equivalent of

        list(A=list(1, NULL), B=list(2, NULL), C=list(NULL, 3))
        '
