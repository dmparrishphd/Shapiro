...
H.FIRST.DAY.OF.MONTH.GREG <- postfix(postfix("-", 1:12 %|% h.pad.i), "-01")

last.day.of.month.greg <- function(leapyear=F)
    31L %,% ic(28, 29)[leapyear %|% index.b] %,% i.h("31 30 31 30 31 31 30 31 30 31")

days.per.month.greg <- last.day.of.month.greg
...
firstofmonths.Dates <- function (dates) {
    ...
    Doc$firstofmonths.Dates <- '
        firstofmonths.Dates computes the range of dates from the
        argument and returns all of the dates in that range
        which are also the first day of the month.'
...
ticks.months.x <- function (...)
    ...
    Doc$firstofmonths.Dates <- '
        firstofmonths.Dates returns the dates within the range
        of the given dates that are also the first of the month.

        > firstofmonths.Dates(as.Date(c("1999-03-25", "1999-08-15")))

        [1] "1999-04-01" "1999-05-01" "1999-06-01" "1999-07-01" "1999-08-01"'
    ...
    Doc$ygreg.inclusive.range.Dates <- '
            ygreg.inclusive.range.dates returns the range of
            Gregorian years (viewed as entire calendar
            years---12 months in duration) during which the
            given dates occur.  Dates are considered to be
            instants in time, and years are considered to
            include the last instant of time so, e.g.,
            ygreg.inclusive.range.dates(as.Date(c("2000-01-01",
