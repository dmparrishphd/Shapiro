...
histogram <- function(i) #TAGS classification
    ...
    Doc$histogram <- '
        ...
        Counts the occurrences of the values in the integer
        vector argument, whose members are assumed to natural
        numbers or NA.  Returns an integer vector where element
        1 is the count of ones in the argument, element 2 is the
        count of twos, and so on.
        
        TIP: pass the result of histogram to matrix. The rows of
        the return together with the row numbers tell both the
        value and the count of the value.
...
df2.histogram.auto <- histogram.auto %O% as.data.frame

    Doc$df2.histogram.auto <- '
        df2.histogram.auto is a simpler version of
        df.histogram.auto. The return has two columns, and any
        names are due to R defaults. The first column contains
        the unique values and the second column contains the
        counts.'
