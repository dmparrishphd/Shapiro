...
mono <- monochrome.colors
    ...
    Doc$monochrome.colors <- '
        monochrome.colors returns a color.vector of length 2,
        where thr first item is TRANSPARENT and the second item is
        equal to the argument.

        Intended for
        use with image_im, and image or plot data where the low
        value (e.g., F, 0L, or 0) maps to no color and the high
        value (e.g., T, 1L, or 1) maps to another color.'
...
dot <- function(x, y, dx=1, dy=1, col="fg" %|% par, add=T, ...) #TAGS image plot
        image_bare(
            x=c(x - dx, x),
            y=c(y - dy, y),
            z=M1,
    ...
    Doc$dot <- '
        ...
        location (x, y) (arg 1, arg 2), the upper-right corner of the dot.
        ...
        2019-09-13
                ...
                col default was previously BLK.

                no longer specifying xlab or ylab internally, as
                these defaults are applied in image_bare.
...
blank.slate <- function (col=par.bg()) #TAGS plot
    ...
    Doc$blank.slate <- '
        Covers the user space of the current device with the
        specified color (default: background color, par("bg")).'
